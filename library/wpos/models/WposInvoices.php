...
class WposInvoices {
    ...
    public function updateItem($result){
        ...
        // update item record
        $itemMdl = new SaleItemsModel();
        if ($itemMdl->edit($this->data->itemid, $this->data->sitemid, 0, $this->data->qty, $this->data->name, $this->data->desc, $this->data->taxid,$this->data->cost, $this->data->tax, $this->data->unit, $this->data->price)===false){
    ...
    private function saveInvoiceData(){
        if ($this->invMdl->edit($this->id, null, json_encode($this->invoice), $this->invoice->status, $this->invoice->discount,$this->invoice->cost, $this->invoice->total, $this->invoice->balance, $this->invoice->processdt,  $this->invoice->duedt)===false){
            return false;
        }
        return true;
    }
