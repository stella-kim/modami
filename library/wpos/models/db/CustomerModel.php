...
class CustomerModel extends DbConfig
{
    ...
    public function create($email, $name, $phone, $mobile, $address, $suburb, $postcode, $state, $country, $gid='')
    {
        ...
        $sql = 'SELECT ifnull(max(id),0) as idx FROM customers';
        $placeholders = [];
        ...
        $sql = "INSERT INTO customers (id,email, name, phone, mobile, address, suburb, postcode, state, country, googleid, dt) VALUES (:id,:email, :name, :phone, :mobile, :address, :suburb, :postcode, :state, :country, :googleid, '".date("Y-m-d H:i:s")."')";
        $placeholders = [":id"=>$idx,":email"=>$email, ":name"=>$name, ":phone"=>$phone, ":mobile"=>$mobile, ":address"=>$address, ":suburb"=>$suburb, ":postcode"=>$postcode, ":state"=>$state, ":country"=>$country, ":googleid"=>$gid];
