...
class StoredItemsModel extends DbConfig
{
    ...
    {
        $sql = 'SELECT ifnull(max(id),0) as idx FROM stored_items';
        $placeholders = [];
        ...
        $sql          = "INSERT INTO stored_items (`id`,`data`, `categoryid`, `code`, `name`, `price`) VALUES (:id,:data, :categoryid, :code, :name, :price);";
        ...
        $placeholders = [":id"=>$idx,":data"=>json_encode($data),":categoryid"=>$data->categoryid, ":code"=>$data->code, ":name"=>$data->name, ":price"=>$data->price ];
