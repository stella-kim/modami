...
class DevicesModel extends DbConfig
{
    ...
    {
        $sql = 'SELECT ifnull(max(id),0) as idx FROM devices';
        $placeholders = [];

        $items = $this->select($sql, $placeholders);
        $idx   = $items[0]['idx']+1;
    
        $sql = "INSERT INTO devices (id,name, locationid, data) VALUES (:id, :name, :locationid, :data)";
        $placeholders = [':id'=>$idx,':name'=>$data->name, ":locationid"=>$data->locationid, ":data"=>json_encode($data)];
    ...
    public function addUuid($uuid, $deviceId){
        ...
        $sql = 'SELECT ifnull(max(id),0) as idx FROM device_map';
        $placeholders = [];
        $items = $this->select($sql, $placeholders);       
        $idx   = $items[0]['idx']+1;

        $sql = "INSERT INTO device_map (id,deviceid, uuid, ip, useragent, dt) VALUES (:id,:deviceid, :uuid, :ip, :useragent, NOW())";
        $placeholders = [':id'=> $idx,':uuid'=>$uuid, ':deviceid'=>$deviceId, ':ip'=>$_SERVER['REMOTE_ADDR'], ':useragent'=>$_SERVER['HTTP_USER_AGENT']];
