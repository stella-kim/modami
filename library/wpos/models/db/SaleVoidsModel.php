...
class SaleVoidsModel extends DbConfig
{
    ...
    {
        $sql = 'SELECT ifnull(max(id),0) as idx FROM sale_voids';
        $placeholders = [];
        $items = $this->select($sql, $placeholders);       
        $idx   = $items[0]['idx']+1;
        
        $sql = "INSERT INTO `sale_voids` (`id`,`saleid`, `userid`, `deviceid`, `locationid`, `reason`, `method`, `amount`, `items`, `void`, `processdt`, `dt`) VALUES (:id,:saleid, :userid, :deviceid, :locationid, :reason, :method, :amount, :items, :isvoid, :processdt, now())";
        $placeholders = [
            ':id'=> $idx,
            ':saleid'=>$saleid,
            ':userid'=>$userid,
