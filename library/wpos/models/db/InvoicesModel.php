...
class InvoicesModel extends TransactionsModel
{
    ...
    {
        $sql = 'SELECT ifnull(max(id),0) as idx FROM sales';
        $placeholders = [];

        $items = $this->select($sql, $placeholders);
        $idx   = $items[0]['idx']+1;
        
        $sql = "INSERT INTO sales (id,ref, type, channel, data, userid, deviceid, locationid, custid, discount, total, balance, status, processdt, duedt, dt) VALUES (:id,:ref, 'invoice', :channel, :data, :userid, :deviceid, :locationid, :custid, :discount, :total, :balance, :status, :processdt, :duedt, '".date("Y-m-d H:i:s")."')";
        $placeholders = [
            ':id'        => $idx,        
            ':ref'        => $ref,
            ':channel'    => $channel,
