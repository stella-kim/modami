...
class AuthModel extends DbConfig
{
    ...
    public function create($username, $password, $isadmin, $permissions)
    {
        ...
        $sql = 'SELECT ifnull(max(id),0) as idx FROM auth';
        $placeholders = [];
        ...
        $sql          = "INSERT INTO auth (id, username, password, uuid, admin, permissions) VALUES (:id,:username,:password,:uuid,:isadmin,:perm)";
        $placeholders = [':id'=>$idx,':username' => $username, ':password' => $password, ':uuid' => $uuid, ':isadmin'=>$isadmin, ':perm'=>$permissions];
