...
class StockHistoryModel extends DbConfig
{
    ...
    public function create($storeditemid, $locationid, $type, $amount, $auxid=-1, $direction=0){
        $sql = 'SELECT ifnull(max(id),0) as idx FROM stock_history';
        $placeholders = [];
        $items = $this->select($sql, $placeholders);       
        $idx   = $items[0]['idx']+1;

        $sql = "INSERT INTO stock_history (id,storeditemid, locationid, auxid, auxdir, type, amount, dt) VALUES (:id,:storeditemid, :locationid, :auxid, :auxdir, :type, :amount, '".date("Y-m-d H:i:s")."');";
        $placeholders = [":id"=> $idx,":storeditemid"=>$storeditemid, ":locationid"=>$locationid, ":auxid"=>$auxid, ":auxdir"=>$direction, ":type"=>$type, ":amount"=>$amount];
