...
function WPOSTransactions() {
    ...
    this.getItemsAll = function(){
        var items = WPOS.sendJsonData("items/get");
        var stocks = WPOS.sendJsonData("stock/get");        
        var tempitem;
        var itemarray = [];                
        for (var i in items){
            tempitem = items[i];
            ...
            for (var key in stocks){
              if (i==stocks[key].storeditemid)
              {  
                tempitem.qty=stocks[key].stocklevel;
                break;
              }            
            }  
    ...
    this.openInvoiceItemDialog = function(index){
        ...
        if (index !== false) {
            ...
            $('#transitemunit1').val(item.unit1);                        
            $('#transitemunit2').val(item.unit2);            
            $('#transitemunit3').val(item.unit3);            
//          $('#transitemunit4').val(item.unit4);            
            $('#transitemunit_').val(item.unit_);
         ...
         // $('#transitemunit').prop("disabled", true); 
//            refreshUnitSelects(item.unit1,item.unit2,item.unit3,item.unit4);                      //hjkim 1111
            refreshUnitSelects(item.unit1,item.unit2,item.unit3);                      //hjkim 1111
//            $('#transitemprice').text(7);
        } else {
    ...
    function setDisabledItemFields() {
        if ($('#transitemsitemid').val() != 0) {
            ...
            $('#transitemdesc').prop("disabled", true);
            $('#transitemunit_').prop("disabled", true);
//hjkim mexico
/*
...
//        var data = {id: curid, sitemid: $("#transitemsitemid").val(), qty: $("#transitemqty").val(), name: $('#transitemname').val(), alt_name: $('#transitemaltname').val(), desc: $('#transitemdesc').val(), unit: $('#transitemunit').val(),unit1: $('#transitemunit1').val(),unit2: $('#transitemunit2').val(),unit3: $('#transitemunit3').val(),unit4: $('#transitemunit4').val(),unit_: $('#transitemunit_').val(), taxid: $('#transitemtaxid').val(), tax: JSON.parse($('#transitemtaxval').val()), price: $('#transitempriceval').val()};
        var data = {id: curid, sitemid: $("#transitemsitemid").val(), qty: $("#transitemqty").val(), name: $('#transitemname').val(), alt_name: $('#transitemaltname').val(), desc: $('#transitemdesc').val(), unit: $('#transitemunit').val(),unit1: $('#transitemunit1').val(),unit2: $('#transitemunit2').val(),unit3: $('#transitemunit3').val(),unit_: $('#transitemunit_').val(), taxid: $('#transitemtaxid').val(), tax: JSON.parse($('#transitemtaxval').val()), price: $('#transitempriceval').val()};
        if (itemid == 0) {
...
*/                    
            ...
            $("#stitemsearch").autocomplete({
                ...
                select: function (event, ui) {
                    ...
                    $('#transitemunit_').val(ui.item.unit);
                    $('#transitemunit1').val(ui.item.price);                    
                    $('#transitemunit2').val(ui.item.price2);
                    $('#transitemunit3').val(ui.item.price3);
//                  $('#transitemunit4').val(ui.item.price4);                    
                    $('#transitemtaxid').val(1);                    
                    // lock fields
                    setDisabledItemFields();
                    calculateItemTotals();
//                    refreshUnitSelects(ui.item.price,ui.item.price2,ui.item.price3,ui.item.price4);                      this.value = "";
                    refreshUnitSelects(ui.item.price,ui.item.price2,ui.item.price3);                      this.value = "";
                    return false;
                }
            });
    ...
    function refreshUnitSelects(price,price2,price3){
        var unitsel = $(".unitselect");
        unitsel.html('');
        unitsel.append('<option value="' + price + '" > ' + price + '  </option>   <option value="' + price2 + '" > ' + price2 + '  </option>  <option value="' + price3 + '" > ' + price3 + '  </option>   ');            
    }
